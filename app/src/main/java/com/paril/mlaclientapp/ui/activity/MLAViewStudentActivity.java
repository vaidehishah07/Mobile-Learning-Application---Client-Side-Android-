...
public class MLAViewStudentActivity extends BaseActivity {
    ...
    class MLAAddStudentAPI extends AsyncTask<Void, Void, String> {
        ...
        @Override
        protected String doInBackground(Void... params) {
            Call<String> callAddStudent = Api.getClient().addStudent(userDetails.getIdStudent(), userDetails.getPassword(), userDetails.getFirstName(), userDetails.getLastName(), userDetails.getTelephone(), userDetails.getAddress(), userDetails.getAliasMailId(), userDetails.getEmailId(), userDetails.getSkypeId());
            try {
                Response<String> respCallAdmin = callAddStudent.execute();
