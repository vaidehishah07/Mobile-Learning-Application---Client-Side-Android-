...
//    KeyRecyclerAdapter listAdapter;
    ...
    class MLAAddAdminAPI extends AsyncTask<Void, Void, String> {
        ...
        @Override
        protected String doInBackground(Void... params) {
            Call<String> callAddAdmin = Api.getClient().addAdmin(userDetails.getIdAdmin(), userDetails.getPassword(), userDetails.getFirstName(), userDetails.getLastName(), userDetails.getTelephone(), userDetails.getAddress(), userDetails.getAliasMailId(), userDetails.getEmailId(), userDetails.getSkypeId());
            try {
